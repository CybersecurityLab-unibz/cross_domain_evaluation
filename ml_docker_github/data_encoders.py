...
class AbstractDataEncoder():
    ...
    def __init__(self, tokenizer, max_length, separate_comments):
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.comment_regex = re.compile('((\/\*([\s\S]*?)\*\/)|(\/\/.*))', re.MULTILINE)
    ...
    def encode_examples(self, ds, limit=-1):
        ...
        for code in tfds.as_numpy(ds):
            code_str = code.decode()
            bert_input = self.convert_example_to_feature(code_str)
...
class SingleTaskDataEncoder(AbstractDataEncoder):
    ...
    def __init__(self, tokenizer, max_length, separate_comments):
        super().__init__(tokenizer, max_length, separate_comments)
