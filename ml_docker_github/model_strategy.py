...
from codebert_tokenizer import CodeBERTTokenizer
...
class ModelStrategy():
    ...
    def create_encoder(self, separate_comments, truncation_side='right'):
        ...
        if self.model_name == 'multitask':
            encoder = MultiTaskDataEncoder(tokenizer, max_length, separate_comments)
        elif self.model_name == 'vulonly':
            encoder = VulOnlyDataEncoder(tokenizer, max_length, separate_comments)
        elif self.model_name == 'satdonly':
            encoder = SATDOnlyDataEncoder(tokenizer, max_length, separate_comments)
        elif self.model_name == 'vulsatd':
            encoder = VulSATDDataEncoder(tokenizer, max_length, separate_comments)
        else:
            print('Unknown algorithm. Valid options are default, hidden and multitask')
        return encoder

    def create_model(self, learning_rate, dropout_prob, l2_reg_lambda, shared_layer, warmup_steps, decay_steps, gamma):
